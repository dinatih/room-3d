...
export function buildGarden(scene) {
  ...
  spawnRow([
    'red_criminal_model_ff_freefire.glb', 'red_robot.glb',
  ], -655, 210);


  // =============================================
  // RANGÉE ANIMATIONS — tous les GLBs des autres rangées qui ont une animation
  // Chargés en parallèle, filtrés, puis positionnés à Z=-810
  // =============================================
  {
    const ALL_FILES = [
      'lara_croft.glb',
    ...
    let done = 0;
    const results = []; // { file, gltf }

    const filteredAnim = HIDE_LARA ? ALL_FILES.filter(f => !f.includes('lara')) : ALL_FILES;
    filteredAnim.forEach(file => {
      gltfLoader.load(`media/${file}`, (gltf) => {
        if (gltf.animations?.length > 0) results.push({ file, gltf });
        done++;
        if (done < filteredAnim.length) return;

        // Tous chargés — positionner les modèles animés
        const startX = 150 - (results.length - 1) * SPACING / 2;
        results.forEach(({ file: f, gltf: g }, i) => {
          const model = g.scene;
          const xPos = startX + i * SPACING;
          const rawBox = new THREE.Box3().setFromObject(model);
          const rawSize = rawBox.getSize(new THREE.Vector3());
          ...
          scene.add(model);
          addGalleryUI(model, g, f, null);
          requestRender();
        });
        console.log(`[anim-row] ${results.length} GLBs avec animation sur ${ALL_FILES.length} testés`);
      }, undefined, () => {
        done++;
      });
    });
