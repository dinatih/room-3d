...
export function buildCasquettes(scene) {
  ...
  loader.load('media/baseball_cap.glb', (gltf) => {
    const source = gltf.scene;
    source.traverse(c => { if (c.isMesh) c.material = redMat; });
    ...
    function placeCap(x, y, z, rx, ry, rz, sc) {
      ...
      cap.castShadow = true;
      cap.traverse(obj => { if (obj.isMesh) obj.layers.set(LAYER_FURNITURE); });
      scene.add(cap);
    }
    ...
    placeCap(282, SUNNERSTA_HEAD_TOP, 271.5, 0, Math.PI, 0, scale20 * 0.9);
