...
export function buildLaptop(scene) {
  const BASE_W = 29.7;   // 29.7cm
  const BASE_D = 22.8;   // 22.8cm
  const BASE_H = 1.6;    // 1.6cm
  const SCREEN_W = 29;   // 29cm
  const SCREEN_D = 19.5; // 19.5cm
  const SCREEN_H = 0.8;  // 0.8cm
  const BEZEL = 0.6;
  const PORT_W = 1.2;
  const PORT_H = 0.6;
  const PORT_D = 3;
  ...
  laptopGroup.position.set(220, 56.25, 170);
  ...
  phoneGroup.position.set(242, DESK_Y, 172);
  ...
  const camBump = new THREE.Mesh(
    new THREE.BoxGeometry(3, 0.2, 3.5),
    camMat
  );
  ...
  mugGroup.position.set(198, DESK_Y, 163);
  ...
  const choco = new THREE.Mesh(
    new THREE.CircleGeometry(innerR - 0.1, 24),
    chocoMat
  );
