...
- **Format des liens de code pour Zed** : Ne JAMAIS utiliser la syntaxe d'ancre `#L<num>` (comme `file:///path#L96`), car Zed l'interprète comme faisant partie intégrante du nom de fichier et tente de créer un fichier inexistant `path#L96`. Pour ouvrir un fichier à une ligne précise dans Zed, utiliser le schéma d'URL officiel de Zed : `zed://file/chemin/absolu:ligne` (ou un lien `file:///chemin/absolu` simple sans fragment `#L`).
