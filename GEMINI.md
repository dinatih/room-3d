...
- **Format des liens de code pour Zed** : Ne JAMAIS utiliser la syntaxe d'ancre `#L<num>` (qui tente de créer un fichier inexistant `path#L96`) ni le schéma `zed://` (non cliquable dans l'interface de chat). Pour ouvrir un fichier à une ligne précise dans Zed, TOUJOURS utiliser le format standard avec deux-points : `[NomFichier.tsx:Ligne](file:///chemin/absolu:Ligne)`.
