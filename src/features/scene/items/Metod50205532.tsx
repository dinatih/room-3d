...
export function Metod50205532({ onSize, ...props }: SceneItemProps) {
  ...
  useLayoutEffect(() => {
    ...
    // Plaque / montant vertical à 13 cm du bord droit (X négatif face au meuble), du bas jusqu'à l'étagère
    if (dividerRef.current) {
      const bottomY = 1.8;
      const topY = shelfY - 0.9;
      const divHeight = topY - bottomY;
      dividerRef.current.position.set(-(halfW - 13), bottomY + divHeight / 2, 0.5);
    }
  ...
  return (
    <group {...props}>
      ...
      {/* Plaque / montant vertical à 13 cm du bord droit du meuble, du bas jusqu'à l'étagère */}
      <mesh ref={dividerRef} position={[-7, 30.45, 0.5]}>
