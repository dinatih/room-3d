...
# 1. Uncheck deform for pelvis bones so they don't cause spikes in shorts
for b in ["DEF-pelvis.L", "DEF-pelvis.R"]:
    bone = rig.data.bones.get(b)
    if bone:
        bone.use_deform = False

# 2. Merge core parts into Lara
...
# 3. Apply Automatic Weights to Lara ONLY
bpy.ops.object.select_all(action='DESELECT')
...
# 4. For accessories, we keep their vertex groups but rename them properly
# Then we just ARMATURE parent them so they don't get auto-weights
mapping = {
    "pelvis": "DEF-spine",
...
for obj in bpy.context.scene.objects:
    if obj.type == 'MESH' and obj.name != "Lara" and not obj.name.startswith("WGT"):
