...
if gltf:
    ...
    print("Arm Bones transformations:")
    for idx, node in enumerate(nodes):
        name = node.get('name', '')
        if 'shoulder' in name.lower() or 'arm' in name.lower() or 'hand' in name.lower():
            if 'thumb' in name.lower() or 'index' in name.lower() or 'middle' in name.lower() or 'ring' in name.lower() or 'pinky' in name.lower():
                continue
