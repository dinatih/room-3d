...
def log(msg):
    print(f"[fbx-pipeline-v2] {msg}", flush=True)

def build_model_from_fbx_v2(target_zip_path, ref_zip_path, out_glb_path, is_reference=False):
    log(f"\n==========================================")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Reset Blender to avoid any database contamination
        bpy.ops.wm.read_factory_settings(use_empty=True)
        
        # 1) Import TARGET first so its bones get the clean, original names!
        ...
        if is_reference:
            # For 01 Bikini itself, just import and export to standardize names without weight transfers
        ...
        # 2) Import Reference FBX second so its bones get .001 suffixes
        ...
        # Detect target scale (meters vs centimeters)
        is_meters = False
        ...
        # Define perfect breast bone coordinates relative to the chest bone head
        # Left Breast
        ...
        # 4) Transfer weights for breast vertex groups from reference body mesh to target body mesh
        log("Transferring weights for breast vertex groups...")
        if tgt_meshes:
            tgt_body_mesh = max(tgt_meshes, key=lambda m: len(m.data.vertices))
            
            # Ensure target mesh has the correct breast vertex groups
            ...
            if ref_meshes:
                ...
                # Select only the target body mesh to run modifier on
        ...
        # 6) Clean up bone and vertex group names
        ...
        # Select all target elements and export
        bpy.ops.object.select_all(action='SELECT')
...
if __name__ == "__main__":
    ...
    for zip_name, glb_name, is_ref in all_zips:
        ...
        try:
            build_model_from_fbx_v2(target_zip, ref_zip, out_glb, is_reference=is_ref)
